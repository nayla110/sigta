...
import { AlertCircle, CheckCircle, Clock, XCircle, Upload, FileText, Lock } from 'lucide-react';
...
export default function TAPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (progressRes.success) {
        ...
        // Redirect jika belum bisa akses TA
        if (!data.can_access_ta) {
          alert('Anda belum bisa mengakses halaman Tugas Akhir. Selesaikan Proposal terlebih dahulu.');
          router.push('/mahasiswa/proposal');
          return;
        }
      }
  ...
  async function handlePosting(idx: number) {
    ...
    const babNumber = idx + 4; // BAB 4 dan 5
  ...
  if (!progress || !progress.can_access_ta) {
    return (
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="bg-red-100 border border-red-300 text-red-800 p-6 rounded-lg text-center">
          <Lock className="w-16 h-16 mx-auto mb-4" />
          <h2 className="text-2xl font-bold mb-2">Akses Ditolak</h2>
          <p>Anda belum bisa mengakses halaman Tugas Akhir.</p>
          <p className="text-sm mt-2">Selesaikan semua BAB Proposal terlebih dahulu.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="bg-gradient-to-r from-purple-600 to-purple-700 text-white p-6 rounded-2xl shadow-lg">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold mb-2">Upload Dokumen Tugas Akhir</h1>
            <p className="text-purple-100">Status: {progress.status}</p>
          </div>
          <div className="text-right">
            <div className="text-4xl font-bold">{progress.current_bab}/{progress.max_bab}</div>
            <p className="text-purple-100 text-sm">BAB Selesai</p>
          </div>
        </div>
        
        <div className="mt-4 bg-purple-800 rounded-full h-3 overflow-hidden">
          <div 
        ...
        <div className="mt-4 flex items-center gap-2 text-sm">
          ...
          <span>
            {progress.can_upload_next 
              ...
              : 'Semua BAB TA sudah diupload'}
          </span>
        </div>
      </div>

      {/* Upload form untuk BAB 4 dan 5 */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {Array.from({ length: 2 }).map((_, idx) => {
          const babNumber = idx + 4;
                  ...
                  <>
                    ...
                    <div className="border border-gray-300 rounded-lg p-3 bg-gray-50 mb-3">
                      ...
                      >
                        ...
                        <button
                          ...
                          disabled={!canUpload}
                          className="px-3 py-1 bg-purple-600 text-white rounded-md text-sm hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                        >
                    ...
                    <div className="flex justify-center">
                      <button
                        ...
                        className="px-6 py-2 bg-purple-600 text-white rounded-full hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                      >
      ...
      <div className="bg-white border rounded-2xl shadow-md p-6">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full table-auto border-collapse">
            ...
            <tbody>
              {dokumenList.filter(d => d.jenis_berkas.includes('TA')).length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-4 py-8 text-center text-gray-500">
                    Belum ada dokumen TA yang diupload
                  </td>
                </tr>
              ) : (
                dokumenList
